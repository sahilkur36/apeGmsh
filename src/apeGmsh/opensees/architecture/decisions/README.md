...
| [0042](0042-render-backend-seam.md) | Rendering-backend seam: `SceneLayer` IR + `RenderBackend` Protocol — render-side mirror of ADR 0026; decouples domain logic from PyVista so `trame` web/Jupyter (and a future ParaView export adapter) drop in behind one seam | Accepted (May 2026; R-A → R-C shipped, R-D in ADR 0044) |
...
| [0044](0044-pick-backend-and-export.md) | `PickBackend` Protocol (optional capability gated by `supports_picking()`) + VTK-free pick IR + server-side web picking + `ParaViewExportBackend` — Phase R-D of the render seam; brings the last VTK-bound domain surface (picking) behind a Protocol | Proposed (2026-05-30; four open questions; slices R-D.1 → R-D.4) |
| [0045](0045-selection-and-pick-contract.md) | Selection & pick contract: `SelectionTarget` IR + `Substrate` + canonical `BBox` + `FilterController` (the `0/1/2/3/4` dim spine) + `SelectableSubstrate` Protocol + serialized `SelectionLog` over an additively-widened `PickBackend` — unifies picking across the model/mesh/results viewers, fixes volume picking + the six-bbox mess, lands above ADR 0044 | Accepted (2026-05-30; from a 13-agent design workflow; 4 forks ratified; slices S0 → S6) |
