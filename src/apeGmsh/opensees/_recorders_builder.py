"""Recorders — declarative recorder spec composite (bridge-relocated).

Phase 9 commit 4 relocates the legacy ``Recorders`` fluent helper from
``apeGmsh.results.spec.declaration`` to the bridge package. The
relocation is mechanical: same fluent surface, same per-category
declaration methods, same ``ResolvedRecorderSpec`` output. The one
behavioural change is :meth:`Recorders.resolve` — per Phase 9 D8 it
drops the ``ndm`` / ``ndf`` kwargs and sources both from the
attached bridge.

This module is **transitional**. It feeds the legacy
``DomainCapture`` / ``LiveRecorders`` / ``LiveMPCO`` consumers until
commits 5/6 retire ``ResolvedRecorderSpec``. New code should prefer
:func:`ops.recorder.declare` (Phase 9 commits 3a/3b/3c) which produces
the typed :class:`apeGmsh.opensees.recorder.RecorderDeclaration`.

The ``apeGmsh.results.spec._resolved`` imports here form a
single-cycle opensees → results layering debt. They retire when
commit 5 lands ``DomainCaptureSpec`` and ``ResolvedRecorderSpec``
...
::
    ...
    ops = g.opensees.bridge          # apeSees instance
    ops.model(ndm=3, ndf=6)
    recorders = Recorders(opensees=ops)
    recorders.nodes(pg="Top", components=["displacement"], dt=0.01)
    spec = recorders.resolve(fem)    # ndm/ndf inferred from bridge

Standalone ``Recorders()`` (no bridge) is supported for declarative
construction and introspection. Calling ``.resolve(fem)`` on a
standalone instance raises with a pointer to the bridge path or to
...
class Recorders:
    """Declarative recorder spec builder (transitional fluent helper).

    Standalone ``Recorders()`` constructs without a bridge and supports
    every declaration method (``nodes`` / ``elements`` / ``gauss`` /
    ``line_stations`` / ``fibers`` / ``layers`` / ``modal``) plus
    introspection (:meth:`categories`, :meth:`components_for`,
    :meth:`shorthands_for`). What standalone instances do **not**
    support is :meth:`resolve` — that requires bridge attachment so
    ``ndm`` and ``ndf`` are unambiguous (Phase 9 D8).

    The fluent helper is being retired by Phase 9: new code should
    declare recorders via :func:`ops.recorder.declare`, which produces
    a :class:`apeGmsh.opensees.recorder.RecorderDeclaration` directly
    on the bridge. ``Recorders`` survives until commits 5/6 retire the
    ``DomainCapture`` / ``LiveRecorders`` / ``LiveMPCO`` consumers of
    ...
    def resolve(self, fem: "FEMData") -> ResolvedRecorderSpec:
        """Resolve all records against a FEMData snapshot.

        Sources ``ndm`` and ``ndf`` from the attached OpenSees bridge
        (Phase 9 D8). The bridge must already have ``ops.model(ndm=,
        ndf=)`` called — otherwise ``ndm``/``ndf`` are ``None`` and
        this method raises with a pointer.

        Steps per record:
        1. Expand shorthand components (``"displacement"`` →
           ``displacement_x/y/z``, clipped to ``ndm``/``ndf``).
        ...
        -------
        ResolvedRecorderSpec
            A snapshot tied to ``fem.snapshot_id``.

        Raises
        ------
        RuntimeError
            If this builder was constructed standalone (no bridge),
            or if the attached bridge has not yet had
            ``ops.model(ndm=, ndf=)`` called. The error message points
            at :func:`ops.recorder.declare` for new code or
            ``Recorders(opensees=ops)`` for the transitional path.
        """
        if self._opensees is None:
            raise RuntimeError(
                "Recorders.resolve(fem): no OpenSees bridge attached. "
                "Standalone Recorders() supports declaration and "
                "introspection only; .resolve() needs a bridge to "
                "source ndm/ndf (Phase 9 D8). Either declare via "
                "ops.recorder.declare(...) — the canonical Phase 9 "
                "path that produces a RecorderDeclaration — or "
                "construct as Recorders(opensees=ops) for the "
                "transitional bridge-bound path."
            )
        ndm = getattr(self._opensees, "_ndm", None)
        ndf = getattr(self._opensees, "_ndf", None)
        if ndm is None or ndf is None:
            raise RuntimeError(
                "Recorders.resolve(fem): the attached OpenSees bridge "
                "has no ndm/ndf set. Call ops.model(ndm=, ndf=) before "
                "resolving (Phase 9 D8 binds ndm/ndf at resolve time)."
            )
    ...
    def _lookup_class_hint_for_pgs(
        ...
        if not elem_assignments:
            return None
        from ._element_capabilities import _ELEM_REGISTRY
        names: set[str] = set()
        for pg_name in pgs:
