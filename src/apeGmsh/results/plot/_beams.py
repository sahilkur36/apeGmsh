...
def axes_from_quaternion(
    ...
    local axes), so ``line_force`` can orient a diagram from the recorder's
    true frame — including the cross-section roll a ``.ladruno``
    ``MODEL/LOCAL_AXES`` carries but node geometry can't recover — instead of
    the :func:`compute_local_axes` geometric guess.
    """
