...
apeGmsh speaks one canonical name per result component, chosen to be
verbose and human-readable. Backend adapters (MPCO, recorder
transcoders, domain capture, OpenSees recorder declarations) translate
to these names on the way in. Users can also write shorthands
(``"displacement"``, ``"stress"``) which expand here, clipped to the
active ``ndm``/``ndf``.

This is the single source of truth for "what components exist."

**Location history.** This module originally lived at
``apeGmsh.results._vocabulary`` (results-side ownership). Phase 9
moved it to the top-level ``apeGmsh._vocabulary`` so that both the
