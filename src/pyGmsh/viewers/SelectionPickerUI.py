...
class SelectionPickerWindow(BaseViewerWindow):
    ...
    def _apply_visual_changes(self) -> None:
        """Reapply colors + rebuild dim=0 glyphs after a pref change."""
        picker = self._picker

        # Rebuild dim=0 glyph actor if point size changed
        if getattr(picker, '_batched', False) and 0 in getattr(picker, '_batch_actors', {}):
            self._rebuild_batched_points()
        elif not getattr(picker, '_batched', False):
            self._rebuild_parametric_points()
    ...
    def _rebuild_batched_points(self) -> None:
        ...
        base_r = diag * 0.003 * max(0.1, picker._point_size / 10.0)
        sphere_src = pv.Sphere(radius=base_r, theta_resolution=8, phi_resolution=8)
    ...
    def _rebuild_parametric_points(self) -> None:
        """Rebuild the parametric dim=0 glyph actor with current point_size."""
        import pyvista as pv
        from .SelectionPicker import _IDLE_POINT

        picker = self._picker
        plotter = picker._plotter

        # Find and remove the old dim=0 glyph actor
        old_actor = None
        for dt, actor in list(picker._id_to_actor.items()):
            if dt[0] == 0:
                old_actor = actor
                break
        if old_actor is not None:
            ...
            for dt in [d for d in picker._id_to_actor if d[0] == 0]:
                del picker._id_to_actor[dt]
            picker._actor_to_id.pop(id(old_actor), None)

        # Rebuild from stored coords
        import numpy as np
        centers = []
        tags = []
        for dt in list(picker._id_to_actor.keys()) + [d for d in getattr(picker, '_param_point_tags', [])]:
            pass  # we lost the centers...

        # Use gmsh to get fresh coords
        centers = []
        tags = []
        ...
        for tag in tags:
            picker._register_actor(actor, (0, tag))
