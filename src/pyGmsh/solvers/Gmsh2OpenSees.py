...
(https://github.com/jaabell/gmsh2opensees) so that pyGmsh users can transfer
a mesh into an active OpenSees model with a single call.

Design philosophy
-----------------
* **Optional** — pyGmsh works perfectly fine without ``gmsh2opensees`` installed.
...
class Gmsh2OpenSees:
    """Thin optional bridge between a live pyGmsh session and OpenSees.

    Accessed as ``g.g2o`` after ``g = pyGmsh(...)``.
    ...
    def transfer(self, *, verbose: bool = False) -> None:
        ...
        * An OpenSees model must already be active::
        ...
        if not self._is_session_active():
            raise RuntimeError(
                "Gmsh is not initialised. Open a pyGmsh/Assembly session "
                "before transferring."
            )
