...
- **Inherited test debt paid:** the schema-pin / parity suites (`test_record_h5_dtype`, `test_record_schema_parity`, `test_compose_schema_2_9_0`) had gone stale at the 2.12.0 bump (they pre-dated the `cpl_*` lane and were red on `main`); they now cover the full lane and the version pin reads from `tests/fixtures/schema.py`.
