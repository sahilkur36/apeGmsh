...
| ID | Status | Summary |
|----|--------|---------|
| LOAD-1 | DONE | **Bridge load consumption — [ADR 0051](../src/apeGmsh/opensees/architecture/decisions/0051-bridge-load-consumption.md) + [plan](plan_bridge_load_consumption.md), SHIPPED.** Loads are **opt-in** via `p.from_model(case)` (deleted `_emit_broker_loads`); geometry groups by **case**, bridge owns **patterns**; **all-nodal** (element-`eleLoad` dropped until further notice); two execution modes (non-staged `ops.analyze` / staged `s.pattern`) with a no-mixing guard (`BridgeError`); `WarnUnconsumedModelLoads` + `ops.ignore_model_loads`; `s.remove_bc` alias. PRs #497 (BL-1 case rename) / #498 (BL-2 from_model + delete auto-emit) / #502 (BL-3 stage patterns) / #503 (BL-4 no-mixing guard + reconciliation) / #504 (BL-5 remove_bc) / BL-6 (this docs round). Element-form + cross-dim gravity (ADR 0050 §5) DEFERRED |
...
the stage-scoped `s.pattern`). No auto-emit; a declared-but-unimported
case warns via `WarnUnconsumedModelLoads` (silence with
`ops.ignore_model_loads(case)`). Element-form (`eleLoad`) consumption
remains DEFERRED — the all-nodal scope means a beam UDL lumps to end
nodes (no fixed-end moments) and beam/shell self-weight needs the
section A/t seam.
